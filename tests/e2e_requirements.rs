...
"#, TEST_GROUP));
    ...
    assert!(result.stdout.contains("Started req-pass"), "stdout: {}", result.stdout);

    // Remediation should NOT have run
    assert!(!std::path::Path::new("/tmp/rig-req-test-marker").exists(),
...
"#, TEST_GROUP));
    ...
    assert_eq!(result.code, 0, "stderr: {}", result.stderr);
    assert!(result.stdout.contains("Started req-fix"), "stdout: {}", result.stdout);

    assert!(std::path::Path::new("/tmp/rig-req-remediated").exists(),
...
"#, TEST_GROUP));
    ...
    assert!(result.stderr.contains("Requirement remediation failed"), "stderr: {}", result.stderr);
}
