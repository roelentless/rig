...
fn watch_config_parses_correctly() {
    ...
    assert_eq!(watched["watch"]["extensions"], serde_json::json!(["txt", "md"]));
    assert_eq!(watched["watch"]["patterns"], serde_json::json!(["**/*.log"]));
    assert_eq!(watched["watch"]["ignore"], serde_json::json!(["**/cache/**"]));
...
"#, TEST_GROUP));
    ...
    assert!(svc["watch"]["paths"].is_null(), "paths should be null when not specified");
}
...
fn watch_service_starts_with_watchexec() {
    ...
    assert_eq!(result.code, 0, "stderr: {}", result.stderr);
    assert!(result.stdout.contains("Started watched-svc"), "stdout: {}", result.stdout);
